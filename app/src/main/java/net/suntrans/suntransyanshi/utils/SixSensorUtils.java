...
/**
 * Created by Looney on 2017/3/1.
 */

public class SixSensorUtils {
    ...
    private static String warning_state[] = new String[]{"1", "1", "1", "1", "1", "1"};   //是否报警，依次是振动、PM2.5、甲醛、烟雾、温度，人员

    public static void parseSixSensorData(List<SixSensorItem> data, String s) {
        ...
        if (IsEffective == 1)   //如果数据有效，则进行解析，并更新页面
        {
            if (s.substring(10, 12).equals("04") || s.substring(10, 12).equals("03"))   //如果是读寄存器状态，则判断是读寄存器1（室内参数），还是寄存器2（灯光信息）的状态
            {
                if (s.substring(16, 18).equals("22") && a.length > 40)  //寄存器1，参数信息，长度34个字节
                {
                    ...
                    data.get(0).setValue(date);
                    data.get(1).setValue(pm1 + "");
                    data.get(2).setValue(pm10 + "");
                    data.get(3).setValue(pm25 + "");
                    data.get(4).setValue(arofene + "");
                    data.get(5).setValue(smoke + "ppm");
                    data.get(6).setValue(tmp + "℃");
                    data.get(7).setValue(humidity + "%RH");
                    data.get(8).setValue(atm + "kPa");
                    data.get(9).setValue(light + "");
                    data.get(10).setValue(staff + "");
                    data.get(11).setValue(xdegree + " °");
                    data.get(12).setValue(ydegree + " °");
                    data.get(13).setValue(zdegree + " °");
...
//                        data.get(1).put("Value", String.valueOf(ydegree) + " °");  //y轴角度
                    if (zdegree > 10) {
                        data.get(13).setEvaluate("倾斜");
                    } else {
                        data.get(13).setEvaluate("正常");
                    }
                    data.get(14).setValue(shake + "");
...
//                    data.get(1).put("Progress", String.valueOf(progress));
                    ...
                    data.get(1).setEvaluate(eva);     //PM1
//                    data.get(1).put("Progress", String.valueOf(progress));
                    ...
                    data.get(3).setEvaluate(eva);    //PM2.5
//                    data.get(3).put("Progress", String.valueOf(progress));
                    ...
                    data.get(2).setEvaluate(eva);    //PM2.5
                    ...
                    data.get(6).setEvaluate(eva);    //PM2.5
                    ...
                    if (progress > 90)
                        progress = 90;
                    data.get(7).setEvaluate(eva);    //PM2.5
                    ...
                    data.get(8).setEvaluate(eva);    //PM2.5


                    data.get(10).setEvaluate(staff == 1 ? "有人" : "无人");     //评估人员信息
                    ...
                    data.get(9).setEvaluate(eva);    //PM2.5
