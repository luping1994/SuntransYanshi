...
public class SwitchControl_fragment extends Fragment implements TcpHelper.OnReceivedListener {
    ...
    private int port;
    private String ip;
    private SwitchItem ipinfo;

    private ProgressDialog connectDialog;

    public static SwitchControl_fragment newInstance(ArrayList<SwitchItem> datas, SwitchItem ipinfo) {
        SwitchControl_fragment fragment = new SwitchControl_fragment();
        Bundle bundle = new Bundle();
        bundle.putParcelableArrayList("datas", datas);
        bundle.putParcelable("item", ipinfo);
...
//        recyclerView.addItemDecoration(new DividerItemDecoration(getActivity(), LinearLayoutManager.VERTICAL));
        ...
        connectToServer(ipinfo.getBendiip(), Integer.valueOf(ipinfo.getBendiport()));
    }
    ...
    private List<SwitchItem> getListDataSet() {
        ...
        ipinfo = getArguments().getParcelable("item");
        addrs = new ArrayList<>();
        for (SwitchItem item :
                datas) {
    ...
    public void onReceive(String content) {
        ...
        if (((St_slc_10_activity) getActivity()).flag.equals("bendi"))
            SwitchUtils.parseSwitchState(datas, content);
        else {
            SwitchUtils.parseSwitchStateNo_addr(datas, content);
